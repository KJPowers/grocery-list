...
#
# Filename: $URL: https://grocery-list.svn.sourceforge.net/svnroot/grocery-list/trunk/listinventory.php $
...
if ($_REQUEST['submit'] == "Update") {
    ...
    foreach ($_SESSION['listitemsstore'] as $key => $value) {
        if ($INVENTORY_NON_ZERO AND floatval($listitemsstore[$key][1]) == 0) {
...
if (isset($_REQUEST['add'])) {
    ...
    if ($INVENTORY_NON_ZERO AND floatval($addsize) == 0) {
...
form_gen($items, "<input type='hidden' name='id' value='$id' /><input type=\"submit\" name=\"add\" value=\"Add to Inventory List\" />");
...
if ($delete) {
    ...
    print '</td>';
    print '<td><input id="count' . $i . '" name="count[]" ' . $disabled . ' maxlength="10" size="3" value="' . $row['count'] . '" />';
    if ($modify) {
